...
test("question, source officielle, relance et nouveau chat", async ({page}) => {
  ...
  await expect(page.getByRole("link",{name:"Lire la fiche officielle"}).first()).toHaveAttribute("href",/^https:\/\/www\.parlement\.brussels\//);
...
test("API rejette les origines tierces et les messages excessifs", async ({request}) => {
  const denied=await request.post("/api/chat",{headers:{origin:"https://evil.example"},data:{message:"STIB"}});
  expect(denied.status()).toBe(403);
  const long=await request.post("/api/chat",{headers:{origin:"http://127.0.0.1:3000"},data:{message:"a".repeat(1501)}});
