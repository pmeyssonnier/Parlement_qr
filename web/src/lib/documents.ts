...
export function safeSourceUrl(value: string) {
  ...
  if (!["www.parlement.brussels", "parlement.brussels", "weblex.brussels"].includes(url.hostname) || url.username || url.password || url.port || !["http:", "https:"].includes(url.protocol)) throw new Error("Source non officielle");
  url.protocol = "https:";
  return url.toString();
}
export function validateCorpus(raw: unknown) {
  ...
  if (new Set(ids).size !== ids.length || new Set(corpus.questions.map(q => q.id)).size !== ids.length) throw new Error("Identifiants en double");
...
export function nature(q: Question): Nature {
  ...
  if (/aucune des questions ne relève|ne relève(?:nt)? pas de (?:ma|la) compétence|adresser toutes vos questions à ma collègue/i.test(q.reponse)) return "incompetence";
...
export function splitText(text: string, max = 2200): string[] {
  ...
  for (const paragraph of text.split(/\n+/).filter(Boolean)) {
    if (current && current.length + paragraph.length + 1 > max) { parts.push(current); current = ""; }
    if (paragraph.length <= max) { current += (current ? "\n" : "") + paragraph; continue; }
    for (const word of paragraph.split(/\s+/)) {
      if (current && current.length + word.length + 1 > max) { parts.push(current); current = ""; }
...
export function passages(q: Question): Passage[] {
  return (["question", "reponse"] as const).flatMap(section => splitText(q[section] || "").map((text, order) => ({ id: `${q.id}:${section}:${order}`, questionId: q.id, section, text, order })));
}
