...
export const questionSchema = z.object({
  ...
  destinataire: z.string(), date_reception: z.iso.date(),
  date_publication: z.iso.date().nullable(), date_reponse: z.iso.date().nullable(),
  question: z.string().min(1), reponse: z.string().nullable(), url_source: z.url(),
});
...
export const sourceSchema = z.object({
  id: z.string(), title: z.string(), author: z.string(), recipient: z.string(),
  date: z.iso.date().nullable(), url: z.url(), excerpt: z.string(), nature: natureSchema,
});
...
export const chatResponseSchema = z.object({
  mode: z.enum(["extraits", "ia"]), status: statusSchema,
  paragraphs: z.array(paragraphSchema), sources: z.array(sourceSchema),
  notice: z.string(), requestId: z.string(),
});
...
export const generatedSchema = z.object({ status: statusSchema, paragraphs: z.array(paragraphSchema), limits: z.string() });
// Enforced server-side on a documented answer before anything is displayed.
export const documentedParagraphsSchema = z.array(z.object({
  text: z.string().max(5000).refine(text => text.trim().length > 0, "Texte vide"),
...
export const searchRowSchema = z.object({
  id: z.string(), question_id: z.string(), section: sectionSchema, content: z.string(),
  position: z.number().int(), score: z.number(), document: questionSchema,
});
