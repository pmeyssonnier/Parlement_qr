...
export function database() {
  ...
  db = url && key ? createClient<Database>(url, key, { auth: { persistSession: false, autoRefreshToken: false } }) : null;
  return db;
}
let ai: OpenAI | undefined;
function openai() { return ai ??= new OpenAI({ apiKey: process.env.OPENAI_API_KEY, maxRetries: 0 }); }
...
export async function corpusInfo() {
  ...
  if (db) {
    ...
    return { count: data.count, answerCount, extractedAt: data.extracted_at, method: data.method, origin: "supabase" as const };
  }
...
async function embed(query: string): Promise<number[] | null> {
  try {
    const result = await openai().embeddings.create({ model: process.env.EMBEDDING_MODEL || "text-embedding-3-small", input: query, dimensions: 1536 }, { timeout: 15000 });
...
export async function findHits(query: string, useEmbeddings: boolean): Promise<Hit[]> {
  ...
  const hits = (data ?? []).map(raw => {
    ...
    return { passage: { id: row.id, questionId: row.question_id, section: row.section, text: row.content, order: row.position }, question: row.document, score: row.score };
  });
