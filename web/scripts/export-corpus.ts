...
async function main() {
  ...
  const payload = {
    schema_version: "1.0",
    extrait_le: version.extracted_at,
    nombre_elements: version.count,
    methode_echantillonnage: version.method,
    questions,
  };
...
main().catch(e => {
  console.error(e instanceof Error ? e.message : "Export impossible");
  process.exitCode = 1;
});
